...
3. Facebook OAuth 화면으로 리다이렉트 (4개 권한 요청)
4. 사용자가 권한 승인
...
### 1.4 요청하는 Facebook/Instagram 권한 4개
...
| 커밋 | 내용 | 왜 |
|------|------|----|
| `f5fa247` | OAuth 스코프를 `email,public_profile` → 4개 Instagram 권한으로 변경, API v21→v22 | **이전 스코프로는 Instagram API 호출 자체가 불가능했음** |
...
3. Facebook 권한 승인 화면 (4개 권한 표시됨) → 승인
...
| "No Instagram Business Account found" | Instagram이 Facebook 페이지에 연결 안 됨 | Instagram 설정 → 페이지 연결 |
